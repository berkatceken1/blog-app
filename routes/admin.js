...
router.post("/blog/create", imageUpload.upload.single("resim"), async function(req, res) {
    ...
    try {
        await db.execute("INSERT INTO blog(baslik, altbaslik, aciklama, resim, categoryid, anasayfa, onay) VALUES(?,?,?,?,?,?,?)", [baslik, altbaslik, aciklama, resim, kategori, anasayfa, onay]);
...
router.post("/blogs/:blogid", imageUpload.upload.single("resim"), async function(req, res) {
    ...
    try {
        await db.execute("UPDATE blog SET resim=?, baslik=?, altbaslik=?, aciklama=?, anasayfa=?, onay=?, categoryid=? WHERE blogid=?", [resim, baslik, altbaslik, aciklama, anasayfa, onay, kategoriid, blogid]);
